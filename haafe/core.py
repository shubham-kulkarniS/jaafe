...
yaafecore.getComponentParameters.restype = POINTER(
    POINTER(ComponentParameters))
yaafecore.getComponentParameters.argtypes = [c_char_p]
yaafecore.getOutputFormatParameters.restype = POINTER(
    POINTER(ComponentParameters))
...
yaafecore.readH5FeatureDescriptions.restype = POINTER(
    POINTER(H5FeatureDescription))
yaafecore.readH5FeatureDescriptions.argtypes = [c_char_p]
yaafecore.freeH5FeatureDescriptions.restype = None
yaafecore.freeH5FeatureDescriptions.argtypes = [POINTER(
    POINTER(H5FeatureDescription))]
...
yaafecore.dataflow_createNode.argtypes = [c_void_p, c_char_p,
                                          POINTER(c_char_p*2)]
...
yaafecore.dataflow_link.argtypes = [c_void_p, c_void_p, c_char_p, c_void_p,
                                    c_char_p]
...
yaafecore.audiofileprocessor_setOutputFormat.argtypes = [c_void_p, c_char_p,
                                                         c_char_p,
                                                         POINTER(c_char_p*2)]
yaafecore.audiofileprocessor_processFile.argtypes = [c_void_p, c_void_p,
                                                     c_char_p]
...
yaafecore.engine_output_available.argtypes = [c_void_p, c_char_p,
...
try:
    ...
    yaafecore.engine_input_write.argtypes = [
        c_void_p, c_char_p, ndpointer(dtype='f8', ndim=2, flags='CONTIGUOUS'),
        c_int, c_int]
    yaafecore.engine_output_read.restype = c_int
    yaafecore.engine_output_read.argtypes = [
        c_void_p, c_char_p, ndpointer(dtype='f8', ndim=2, flags='CONTIGUOUS'),
        c_int, c_int]
except ImportError:
    # miss some functions
