...
class AudioFeature(object):
    ...
    def filter_params(cls, params):
        '''
        Returns a dictionary holding only parameters defined from the PARAMS
        attribute with default value for those not in the given params dict.
        '''
        return dict([(name, params.get(name, default))
                    for (name, default, desc)
                    in cls._EXPOSED_PARAM_DESCRIPTORS])
