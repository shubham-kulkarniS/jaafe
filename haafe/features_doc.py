...
def generate_features_parameters(app, what_, name, obj, options, lines):
    if (what_ == 'class' and issubclass(obj, AudioFeature) and
        ...
        seealso = [step[0] for step in obj.PARAMS if isinstance(step[0], type)
                   and issubclass(step[0], AudioFeature)]
        if seealso:
            seealso = ', '.join([':class:`%s`' % cls.__name__
                                for cls in seealso])
            lines.append('')
            lines.append('.. seealso:: %s' % seealso)

    elif what_ == 'module':
        allFeatures = [kls for kls in obj.__dict__.values()
                       if isclass(kls) and issubclass(kls, AudioFeature)
                       and not kls == AudioFeature]
        if not allFeatures:
            return
        simpleFeature = [kls.__name__ for kls in allFeatures
                         if not kls.TRANSFORM]
